...
module.exports = {
  ...
  async execute(interaction) {
    ...
    const response2 = await interaction.followUp({
      content:
        "choose which pookies you want from " + r.username + "'s inventory",
      components: [row2],
    });
    ...
    const filter2 = (i) =>
      i.user.id === interaction.user.id && i.customId === interaction.id + "2";
    const filter3 = (i) =>
      i.user.id === interaction.user.id && i.customId === interaction.id + "3";
