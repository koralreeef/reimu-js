...
async function sumTotal() {
    ...
    for (let i = 0; i < users.length; i++) {
        ...
        const user = users[i];
        const pookies = await user.getPookies(user.user_id);
        const map = pookies.map((i) => i.amount);
        const sumWithInitial = map.reduce((accumulator, currentValue) => accumulator + currentValue, initialValue);
...
module.exports = {
    ...
    async execute(client) {
        ...
        client.user.setActivity("activity", {
            ...
            name: "custom",
            state: getTotalPookies() + " pookies down in the fantastic blowhole",
        });
        console.log(getTotalPookies() + " pookies logged on startup.");
        // check if pookies arent same
        setInterval(async () => {
            const newsum = await sumTotal();
