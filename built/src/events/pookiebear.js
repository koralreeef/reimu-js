...
import { SNOW_EXTRA_SSR_CHANCE, RAIN_EXTRA_ROLL_CHANCE, } from "../constants/weatherConstants.js";
...
module.exports = {
    ...
    async execute(message) {
        ...
        const pookieVilleMultiplier = userLocation === "pookieville" ? POOKIEVILLE_EXTRA_ROLL_CHANCE : 0;
        const pookieChance = h.commonSR + rainMultiplier + pookieVilleMultiplier;
        const starryPookie = h.getStarnight() ||
            (userLocation === "star peak" && Math.random() >= 0.7);
        const ssrPookie = h.SSR + snowMultiplier > h.getRandomInt(1000);
        if (pookieChance < h.getRandomInt(100))
            return;
        const pookie = h.getHurricane()
            ? await getHurricanePookie()
            : await rollPookie(starryPookie, ssrPookie);
        const pookiesToSummon = userLocation === "pookie forest" && Math.random() >= 0.5 ? 2 : 1;
        u.addPookies(pookie, userID, pookiesToSummon, pookie.rarity);
        Pookiebears.update({ summon_count: pookie.summon_count + pookiesToSummon }, { where: { id: pookie.id } });
        pookie.summon_count += pookiesToSummon;
        const messagePayload = buildEmbed(pookie, message.author.username, h.getBalance(userID), pookiesToSummon);
