...
const { locationMap, getEmbedColor, tierMap, starry, } = require("../../helper.js");
...
async function makePlusPookie(name, fileName, avatarURL, username, rarity, source, tier) {
    //enum for rewards?
    let plus = "+";
    if (tier == 4)
        plus = "++";
...
module.exports = {
    ...
    async execute(interaction) {
        ...
        const pookieEmbed = await buildEmbed(user, pookie, quest.questTier, quest.due_amount);
            ...
            try {
                ...
                if (confirmation.customId === user.user_id + "yes" + epoch) {
                    ...
                    if (pookie) {
                        ...
                        pookie = await makeStarryPookie(starName, pookieFileName, avatarURL, userUsername, p.rarity, p.source);
                        user.addPookies(pookie, id, quest.reward_amount, pookie.rarity);
                    }
                    else if (quest.questTier > 1) {
                        const p = await Pookiebears.findOne({
                        ...
                        const avatarURL = interaction.user.displayAvatarURL();
                        const userUsername = interaction.user.username;
                        pookie = await makePlusPookie(name, pookieFileName, avatarURL, userUsername, p.rarity, p.source, quest.questTier);
                        user.addPookies(pookie, id, quest.reward_amount, pookie.rarity);
                    }
                    let newcount;
                    let turnedin = false;
                    if (quest.questTier == user.questTier ||
                        quest.questTier == user.questTier - 1) {
                        newcount = user.questLifetime + 1;
                        user.update({ questLifetime: newcount }, { where: { user_id: user.id } });
                        turnedin = true;
                    }
                    ...
                    if (user.questLifetime % 5 == 0 && turnedin == true) {
                        ...
                        user.update({ questTier: newcount }, { where: { user_id: user.id } });
                    ...
                    const attachment = new AttachmentBuilder(pookie.file_path);
                    const pookieEmbed = await buildCompleteEmbed(pookie, quest.questTier, quest.reward_amount);
                    quest.destroy();
