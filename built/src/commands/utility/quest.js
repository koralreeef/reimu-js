...
async function amountGenerator(tier, p) {
    ...
    if (tier == 3) {
        ...
        if (p.rarity == starry)
            amount = (amount / 6).toFixed(0);
        if (p.rarity == starry_ssr)
            amount = (amount / 8).toFixed(0);
    }
    if (tier == 4) {
...
module.exports = {
    ...
    async execute(interaction) {
        ...
        const tier = Math.min(interaction.options.getInteger("questtier") ?? user.questTier, 4);
        ...
        if (tier > user.questTier) {
            return await interaction.reply("your quest tier isnt high enough! (" +
                user.questTier +
                " < " +
                tier +
                ")");
            ...
            collector.on("end", async () => {
                switch (questState) {
                    case "PENDING":
                        ...
                        break;
                    case "ACCEPTED": {
                        await acceptQuest(currentQuestPookie, user, tier, amount, lastInteraction);
                        break;
                    }
                    case "DENIED":
...
let acceptQuest = async (currentQuestPookie, user, tier, amount, lastInteraction) => {
