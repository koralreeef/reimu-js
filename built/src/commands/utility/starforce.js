...
module.exports = {
    ...
    async execute(interaction) {
        ...
        if (u.location == "star peak")
            starMultiplier = starMultiplier + 10;
        let allIn = 0;
        let str = "the";
        let wincount = 0;
        const loss = -amount;
        ...
        if ((await check) == false) {
            ...
            if (rolls == 0) {
                return interaction.reply({
                    content: "pookies cannot be greater than amount given (ex. 5 amount/6 pookies)",
                    ephemeral: true,
                });
            }
            ...
            for (let i = 0; i < rolls; i++) {
                ...
                if (roll > rollToBeat) {
                    ...
                    await Pookiebears.update({ summon_count: newCount }, { where: { id: newpookie.id } });
            ...
            if (wincount > 0) {
                ...
                return interaction.editReply({
                    content: "unlucky... you just boomed " +
                        amount +
                        " " +
                        pookie.pookie_name +
                        "(s) away....\nroll to beat: " +
