...
async function buildEmbed(user, username, fav, embedColor, result, pageNumber, pageCount, sort) {
    console.log(sort);
...
module.exports = {
    data: new SlashCommandBuilder()
        ...
        .setName("sorting")
        .addChoices({ name: "ascending rarity", value: "ASC" }, { name: "descending rarity", value: "DESC" })
    ...
    async execute(interaction) {
        ...
        if (unlucky == null)
            return interaction.reply(`${target.tag} has nothing!`);
        ...
        const invEmbed = await buildEmbed(user, target.tag, fav, embedColor, result, 1, pageCount, sort);
        ...
        console.log("total pookies: " + totalPookies + "\npage numbers: " + pageCount);
        ...
        collector.on("collect", async (i) => {
            ...
            if (i.customId === user.user_id + "pagebackward" + epoch) {
                ...
                const invEmbed = await buildEmbed(user, target.tag, fav, embedColor, result, index, pageCount, sort);
                await i.update({
                    content: "",
                    embeds: [invEmbed],
                    files: [attachment],
