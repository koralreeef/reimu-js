...
module.exports = {
    data: new SlashCommandBuilder()
        ...
        .setName("mechanics")
        .setDescription("what mechanic of this lovely world would you like to know about")
        .addChoices({ name: "pookiebears", value: "p" }, { name: "quests", value: "q" }, { name: "starforce", value: "s" }, { name: "locations", value: "l" }, { name: "weather", value: "w" }, { name: "trading", value: "t" })),
    async execute(interaction) {
        const option = interaction.options.getString("mechanics");
        if (option == "p") {
            return await interaction.reply("this is an attempt of making a sorta gacha game but with user submitted characters or 'pookiebears'.\nif you send a chat message with \"pookiebear\" only," +
        ...
        if (option == "q") {
            return await interaction.reply("do enough quests with /quest and your quest tier goes up! your quest tier lets you travel to more locations that help with pookie gathering.\n" +
        ...
        if (option == "s") {
            return await interaction.reply("starforcing your pookies with /starforce gives them pluses at the end of their name! it also booms (destroys) the pookie if the starforce fails.\n" +
        ...
        if (option == "w") {
            return await interaction.reply("weather can have crazy effects whenever you're manually hunting for pookiebears. rainy increases pookiebear chances overall, \n" +
        ...
        if (option == "t") {
            return await interaction.reply("you can trade and/or give away pookies with /tradepookie and /givepookie respectively. work together to fulfill those quests!");
        }
        return await interaction.reply("spam pookiebear in a channel and see what happens!");
    },
};
