...
module.exports = {
  ...
  async execute(message) {
    ...
    if (regex.test(msg) || self) {
      ...
      const selfName = await osuUsers.findOne({
      ...
      try {
        ...
        const mapCS = "CS:  " + map.diff_size;
        const mapAR = "  AR:  " + map.diff_approach.toFixed(2);
        const mapOD = "  OD:  " + map.diff_overall.toFixed(2);
        const mapBPM = "\nBPM:  " + map.bpm.toFixed(2);
        const mapLength = "  Length:  " + getLength(map.hit_length);
        const dtLength = dt.length(map.hit_length);
        const htLength = ht.length(map.hit_length);

        // console.log(map.difficultyrating);
        const accuracy =
          calcAccuracy.osu(rs.count300, rs.count100, rs.count50, rs.countmiss) *
          100;
        const diffValues = findMapStats(
          mapCS,
          mapAR,
          mapOD,
          mapBPM,
        ...
        const timestamp = Math.floor(date / 1000) - 7 * 60 * 60;

        const total = map.count_slider + map.count_normal + map.count_spinner;
        ...
        const rsEmbed = new EmbedBuilder()
          ...
          .setColor(coral)
          .setFooter({
            text: map.approved + " mapset by " + map.creator,
        ...
        return message.channel.send({
          content: "no scores set in 24 hours or user doesnt exist",
          ephemeral: true,
        });
      }
    }
