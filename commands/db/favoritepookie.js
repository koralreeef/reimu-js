...
module.exports = {
	...
	async execute(interaction) {
        ...
        const userPookie = await Pookiebears.findOne({ where: {pookie_name: pookie}})
        const user = await Users.findOne({ where: {user_id: interaction.user.id}});
        const p = await UserPookies.findOne({ where: {pookie_id: userPookie.id, user_id: interaction.user.id}});
        console.log(userPookie);
        if(p)
        { 
            user.update({ favoritePookie: userPookie.pookie_name }, 
                { where: {user_id: user.id} 
             });
             return await interaction.reply("favorite pookiebear set to "+userPookie.pookie_name);
        } else {
