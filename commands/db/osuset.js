  ...
  data: new SlashCommandBuilder()
    ...
    .addStringOption((option) =>
      option
        .setName("name")
        .setDescription(
          "what is your osu name (automatically overwrites previous name",
        )
        .setRequired(true),
    ),
  ...
  async execute(interaction) {
    ...
    if (u) {
      if (!check) {
        ...
        osuUsers.update({ username: osuName }, { where: { user_id: id } });
      }
      return interaction.reply(
        "updated " + interaction.user.username + " to be " + osuName + "!",
      );
    } else {
