...
module.exports = {
  data: new SlashCommandBuilder()
    .setName("starforce")
    .setDescription(
      "summon a special version of your pookie (use /starforce calc for odds)",
    )
    ...
    .addIntegerOption((option) =>
      option
        .setName("pookies")
        .setDescription(
          "how many pookies per roll (ex. 20 amount/ 5 pookies = 4; 4 rolls",
        )
        .setMinValue(1)
        .setMaxValue(25)
        .setRequired(true),
  ...
  async execute(interaction) {
    ...
    if ((await check) == false) {
      ...
      if (rolls == 0) {
        return interaction.reply({
          content:
            "pookies cannot be greater than amount given (ex. 5 amount/6 pookies)",
          ephemeral: true,
        });
      }
      ...
      for (let i = 0; i < rolls; i++) {
        ...
        if (roll > rollToBeat) {
          ...
          newpookie = await Pookiebears.findOne({
            where: { pookie_name: pookie.pookie_name + "+" },
          });
          if (newpookie) {
