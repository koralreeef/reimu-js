...
const { downloadFile, common, ssr, starry, starry_ssr } = require(
  `../../helper.js`,
);
  ...
  async execute(interaction) {
    ...
    if (!url.startsWith(`https://cdn.discordapp.com/attachments/`)) {
      return await interaction.reply({
        content: `only discord image links will be accepted (post your image in a channel and copy its link)`,
        ephemeral: true,
      });
    }
    ...
    try {
      ...
      return await interaction.client.users.send(
        `109299841519099904`,
        `${user.username} just sent this epic pookiebear hell yaah brother \n${url}`,
      );
    } catch (error) {
      console.log(error);
