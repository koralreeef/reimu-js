...
async function buildEmbed(pookie, tier, amount){
	...
	.setColor(embedColor)
	.setFooter({ text: `reward: `+reward+` `+pookie.pookie_name+` `+tierMap.get(tier), 
...
module.exports = {
	...
	async execute(interaction) {
        ...
        .addComponents(accept, deny);
            ...
            const pookie = await Pookiebears.findOne({ where: { id: quest.pookie_id }});
            ...
            try {
                ...
                if (confirmation.customId === 'yes'){
                       ...
                       let pookie = await Pookiebears.findOne({where: {pookie_name: reward}});
                       if(pookie){ 
                        user.addPookies(pookie, id, quest.reward_amount, pookie.rarity)        
                       } 
                        else if(quest.questTier == 1){
                        const p = await Pookiebears.findOne({where: {id: quest.pookie_id}});
                        let starName = p.pookie_name;
                        let pookieFileName = p.file_path.substring(9);
                        let avatarURL = interaction.user.displayAvatarURL();
                        let userUsername = interaction.user.username;

                        pookie = await makeStarryPookie(starName, pookieFileName, avatarURL, userUsername, p.rarity);
                        user.addPookies(pookie, id, quest.reward_amount, pookie.rarity)
                        }   

                        else if(quest.questTier > 1){
                        const p = await Pookiebears.findOne({where: {id: quest.pookie_id}});
                        let name = p.pookie_name;
                        let pookieFileName = p.file_path.substring(9);
                        let avatarURL = interaction.user.displayAvatarURL();
                        let userUsername = interaction.user.username;
                       ...
                       if(user.questLifetime%5 == 0 && turnedin == true) {
                        ...
                        let location = locationMap.get(user.questTier);
                        if(user.questTier == 1)
                       ...
                       quest.destroy();
                       await confirmation.update({content: "**quest complete!**\n"+locationString,
                                      embeds: [pookieEmbed],
                                      files: [attachment],
                                      components: []
                    })
